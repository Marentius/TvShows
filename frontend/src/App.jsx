...
const TVSeriesList = () => {
    ...
    return (
        <Stack spacing={4}>
            {tvShows.map(tvShow => (
                <Box key={tvShow.title} p={4} shadow="md" borderWidth="1px" display="flex" alignItems="center">
                    {tvShow.imageUrl ? (
                        <Image src={tvShow.imageUrl} alt={tvShow.title} boxSize="100px" objectFit="cover" mr={4} />
                    ) : (
                    ...
                    <Box>
                        ...
                        <EpisodesList tvShowId={tvShow.title} />  {/* Viser episoder for hver TV-serie */}
                    </Box>
                </Box>
            ))}
        </Stack>
    ...
    return (
        <Stack spacing={2} mt={4}>
            {episodes.map(episode => (
                <Box key={episode.episodeNumber} p={3} shadow="md" borderWidth="1px">
                    <Text fontWeight="bold">Episode {episode.episodeNumber}: {episode.title}</Text>
                    <Text>{episode.description}</Text>
                    {/* Slett episode-knapp */}
                    <Button colorScheme="red" mt={2} onClick={() => deleteEpisode(tvShowId, episode.seasonNumber, episode.episodeNumber)}>
                        Delete Episode
                    </Button>
                </Box>
            ))}
